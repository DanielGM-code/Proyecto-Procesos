 ...
 * @author angel
 */
public class FXML_ListaCursosConsultaController implements Initializable {
    ...
    FXML_ListaCursosConsultaController(String numeroPersonal) {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            Logger.getLogger(FXML_ListaCursosConsultaController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }    
    ...
    private void seleccionarCurso(MouseEvent event) {
        ...
        if(curso != null){
            Ventana.iniciarVentana(buttonRegresar, new FXML_PlanCursoController(curso, numeroPersonal), "/sistemadegestióndeplanesdecurso/gui/vistas/FXML_PlanCurso.fxml", Ventana.CERRAR);
        }
    }
    ...
    private void llenarTablaCursos() throws SQLException{
        ...
        List<Curso> cursosConPlan = new ArrayList<Curso>();
        ...
        for(Curso cursoActivo : cursosActivos){
            ...
            if(tienePlan){
                cursosConPlan.add(cursoActivo);
            }
        }
        
        List<Curso> cursosDelDocente = new ArrayList<Curso>();
        for(Curso cursoConPlan : cursosConPlan){
            if(cursoConPlan.getDocenteAsignado().equalsIgnoreCase(numeroPersonal)){
                cursosDelDocente.add(cursoConPlan);
            }
        }
